...
* (Debug Console) Page redesigned into three sub-tabs: **Settings** (enable, log level, max entries, script runner), **Categories** (full-width grouped checkboxes with hover tooltips), and **Live Log** (full-width log viewer with refresh, clear, and copy buttons). Descriptions no longer overflow the panel and the log viewer has much more reading room. The `/df console` slash command continues to open the Live Log tab.
